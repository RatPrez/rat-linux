...
# deselected, in which case the user opted out of `rat update` / `rat nvidia`
# entirely (the picker warns before letting that happen).
...
if ! cat_is_selected "updater" "rat-cli" "$CATEGORIES_DIR/updater.toml"; then
  warn "Updater category deselected; not linking the rat CLI. No 'rat update' or 'rat nvidia' on this machine."
